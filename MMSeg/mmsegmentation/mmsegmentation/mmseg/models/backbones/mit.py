...
class MixFFN(BaseModule):
    ...
    def __init__(self,
        ...
        fc1 = conv_block_DDConv(
            ch_in=in_channels,
            ch_out=feedforward_channels
        )
        # 3x3 depth wise conv to provide positional encode information  //位置编码信息
        pe_conv = nn.Sequential(
            nn.Conv2d(
                in_channels=feedforward_channels,
                out_channels=feedforward_channels,
                kernel_size=3,
                stride=1,
                padding=1,  # 由于深度卷积不改变特征图大小，因此 padding 可以设置为 1
        ...
        fc2 = conv_block_DDConv(
            ch_in=feedforward_channels,
            ch_out=in_channels
        )
...
class conv_block_DDConv(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            DDConv(ch_in, ch_out, kernel_size=1, stride=1, padding=0),

            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),

            DDConv(ch_out, ch_out, kernel_size=1, stride=1, padding=0),
